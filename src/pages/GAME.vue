<template>
<div>
    <h1 class="container is-fluid is-size-3">GAME</h1>
    <br>
    <br>
    <h2 class="container is-fluid is-size-4">達成感味わっていますか？</h2>
    <div class="columns">
        <div class="column is-two-five">
            <div>
                <br>
                <img class="gif" src=".././assets/ACHIEVE.jpg"  alt="サムネイル画像">      
            </div>
        </div>
        <div class="column is-thirds-five write">
            <div>
                <br>
                みなさま、人生において忘れられない達成感ってありませんか？
                <br>
                <br>
                プロジェクトが無事終わったとき、試験に合格したとき、
                リーダーの役目を無事終えたとき、いろいろありますよね。
                <br>
                <br>                               
                やっぱり苦労して、苦労してやり切ったときの達成感って
                何者にも代えられない感情だと僕は思っております。
                <br>
                <br>
                <br>
                <br>
                <strong>             
                そんな尋常じゃない達成感を味わえるゲーム、それがこれ『ダークソウル』です！
                PS3で発売されましたが、あまりの人気にPS4でリメイク版が発売されました。 
                </strong>
                <br>
                <br>
                全世界でのシリーズ累計販売本数は２７００万本以上です。
                （ちなみにモンスターハンターの全世界シリーズ累計販売本数は
                ６２００万本以上なので、人気は結構ある方だと思います）
                <br>
                <br>
                僕は大学生のとき、何気なく友達の家でやってからドハマりして、
                気づけばファン歴10年のベテランプレイヤーです。
                <br>
                <br>
                <br>
                <strong>
                ★ダークソウル紹介動画
                </strong>
                <br>
                <div class="column video youtube">
                    <iframe
                    src="https://www.youtube.com/embed/nwJbcN5uxNQ"
                    frameborder="0"
                    allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                    allowfullscreen>
                    </iframe>
                </div>
            </div>
        </div>
    </div>
    <h2 class="container is-fluid is-size-4">絶望的なゲーム仕様</h2>
    <div class="columns">
        <div class="column is-two-five">
            <div>
                <br>
                <img class="gif" src=".././assets/DESPAIR.jpg"  alt="サムネイル画像">      
            </div>
        </div>
        <div class="column is-thirds-five write">
            <div>
                <br>
                少し紹介させていただきますと、ジャンルとしてはアクションRPGで、
                キャラをレベルアップさせて敵を倒していくという話です。
                <br>
                <br>
                なんだ、普通のゲームじゃんて思いますよね。
                しかし、このゲームエグい仕様があるんです。
                <br>
                <br>
                敵を倒すと、ソウルというレベルアップに
                必要なお金的なものがたまっていくのですが、
                プレイヤーが死んだ際にソウルを死んだ場所に落としてしまいます。
                <br>
                <br>
                なので蘇ったプレイヤーは、そのソウルを拾いにいかなければなりませんが、
                そのソウルを拾えずに死んでしまうと、貯まったソウルは失われてしまうんです。
                <br>
                <br>
                つまりどういうことかというと、拾う力がなければ、ソウルを貯めることが出来ず、
                永遠にレベルアップできないということなんです。
                <br>
                <br>
                <br>
                <strong>
                こういったエグい仕様から、このゲームは別名こう呼ばれています、、『心折れソウル』
                </strong>
            </div>
        </div>
    </div>
    <h2 class="container is-fluid is-size-4">試行錯誤の連続</h2>
    <div class="columns">
        <div class="column is-two-five">
            <div>
                <br>
                <img class="gif" src=".././assets/TRY.jpg"  alt="サムネイル画像">      
            </div>
        </div>
        <div class="column is-thirds-five write">
            <div>
                <br>
                他にも絶望的な要素がたくさんありますが、
                ソウルを失わないためにも試行錯誤を繰り返し、
                少しづつ前に進んでいくことが大事になってきます。
                <br>
                <br>                
                そして、
                <strong>
                その過程や絶望を乗り越えた先には、
                今まで試行錯誤した分以上の達成感が必ず待っています！
                </strong>
                <br>
                <br>  
                最初は苦しいですが、慣れてくると、この感覚が病みつきになるほど楽しいんです。
                この計算し尽された達成感こそ、『ダークソウル』の一番の魅力だと思っております。
            </div>
        </div>
    </div>
    <h2 class="container is-fluid is-size-4">絶望、挫折の先にある達成感は言葉では言い表せない・・！</h2>
    <div class="columns">
        <div class="column is-two-five">
            <div>
                <br>
                <img class="gif" src=".././assets/ACIEVEMENT.jpg"  alt="サムネイル画像">      
            </div>
        </div>
        <div class="column is-thirds-five write">
            <div>
                <br>
                僕の場合は、ある敵（O.S)を倒したときの達成感と解放感があまりに凄すぎて、
                その後の居酒屋で一人だけ、ずっと放心状態だったのが良い思い出です。
                あの日、僕の心はダークソウルの世界に残されたままでした。（笑） 
                <br>
                <br>
                ソウルを失うたびに、発狂していく友達を何人も見てきましたが、
                必ず最後はみんなやって良かった。世界観が変わったと言ってくれます。
                <br>
                <br>
                <strong>
                僕自身もこのゲームのおかげで、以前より辛いことから逃げなくなったと自負しております。
                </strong>
                <br>
                <br>
                動機は何であれ、是非やってみたい、鋼のメンタルを手に入れたいという方は、
                伊藤までご連絡ください。PS4版ソフトを無期限でお貸しいたします。  
                コロナ禍の状況でも、家で楽しめること間違いなしです！
            </div>
        </div>
    </div>
    <br>
    <div class="columns is-centered">
        <img class="gif" src=".././assets/FARAN.gif" alt="画像を表示出来ません">
    </div>
    <!-- <br>
    <h2 class="container is-fluid is-size-4">海外でも大人気</h2>
    <br>
    <div class="columns is-desktop is-mobile is-tablet is-centered">
        <div class="video">
            <iframe
            src="https://www.youtube.com/embed/bzJDimvPW1Y"
            frameborder="0"
            allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
            allowfullscreen>
            </iframe>
        </div>
    </div> -->
    <br>
</div>
</template>

<style>
.video{
position: relative;
  width: 70%;
  padding-bottom: 20.25%;
  top: 0;
  left: 0;
  background-color: aqua;
}

.video iframe{
  position: absolute;
  top: 0;
  left: 0;
  width: 100% !important;
  height: 100% !important;
    /* display : block;
    margin : 0 auto; */
}

.youtube{
     display : block;
    margin : 0 auto;
}
</style>
